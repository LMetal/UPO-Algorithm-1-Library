...
{
  //printf("\n\neval -> %s\n", expr);
  upo_stack_t postfix = upo_stack_create();
  long * a;
  long * b;
  long * c;
  char op;
  ...
  for(size_t i=0; i<strlen(expr); i++){
      ...
      if(isdigit(*(expr+i))){
          ...
          sscanf(expr+i, "%ld", a);
          if(*a > 9) i++;
          upo_stack_push(postfix, a);
      }
      else{
          b = (long *)upo_stack_top(postfix);
          upo_stack_pop(postfix, 0);
          a = (long *)upo_stack_top(postfix);
          upo_stack_pop(postfix, 0);
          sscanf(expr+i, "%c", &op);
          ...
          //printf("%ld\n", *res)
          *res = *c;
          upo_stack_push(postfix, c);

          free(a);
          free(b);
      }
  }
  upo_stack_destroy(postfix, 1);
  return 1;
}
